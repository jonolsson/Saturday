...
abstract class api_exceptionhandler_base {
    ...
    public function dispatch($data) {
        try {
            $view = new api_view(array(), "error/error.php", "error.php");
            $view->data = $data;
            $this->context->setView($view);
            $content = $this->context->renderView();
            //$layout = $this->route['view']; // Or some default
            $layout = new api_layout(array(), "error.php"); //$this->route);
            $this->context->setLayout($layout);
//            $this->context->loadLayout();
            $this->context->setLayoutContent($content);
            $content = $this->context->renderLayout();
            echo $content;
            /*$this->context->setController("error", "error");
            $this->context->setControllerData($data);
            $this->context->dispatch();
            */
            // Get registry $request = api_request::getInstance();
            // Stor error data in registry $request->
            /*
            $this->context->loadController();
            $this->context->loadView(); //prepare();
            $this->context->renderView(); //dispatch($data);
            */
        } catch (Exception $e) {
            $this->dump($e);
        }
